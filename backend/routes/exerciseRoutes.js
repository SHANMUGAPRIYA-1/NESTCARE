...
/* ─────────────────────────────────────────────
   ROUTE: GET /api/exercise/latest/:userId
   Returns the most recent exercise session for
   the given user, plus a calculated postpartum
   period (months + days since report upload).
───────────────────────────────────────────── */
router.get('/latest/:userId', async (req, res) => {
  try {
    const session = await Exercise.findOne({ userId: req.params.userId })
      .sort({ createdAt: -1 });

    if (!session) {
      return res.status(404).json({ success: false, error: 'No report found for this user.' });
    }

    // Calculate how long ago the report was uploaded
    const now = new Date();
    const uploadedAt = new Date(session.createdAt);
    const diffMs = now - uploadedAt;
    const diffDays = Math.floor(diffMs / (1000 * 60 * 60 * 24));
    const monthsSinceUpload = Math.floor(diffDays / 30);
    const remainingDays = diffDays % 30;

    // Determine postpartum stage based on days since upload
    let currentStage;
    if (diffDays < 14) currentStage = '1-2';
    else if (diffDays < 42) currentStage = '3-6';
    else if (diffDays < 84) currentStage = '7-12';
    else currentStage = '13+';
    ...
    res.status(200).json({
      ...
      postpartumPeriod: {
        ...
        remainingDays,
        totalDays: diffDays,
        currentStage,        // recalculated stage based on today's date
        originalStage: session.postpartumWeek, // what was in the report
        uploadedAt: session.createdAt,
      },
    });
  } catch (err) {
    console.error('Latest session fetch error:', err);
